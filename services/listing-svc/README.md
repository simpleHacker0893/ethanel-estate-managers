# listing-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** marketplace listings, viewings, saved searches and WhatsApp alerts.

## Contract every service honours

- Owns exactly one Postgres schema (`listing`) and one DB role (`listing_svc`).
