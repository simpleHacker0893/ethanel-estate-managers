# payments-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** M-Pesa STK push, Paybill and Till receipts, reconciliation against expected rent, landlord payouts.

## Contract every service honours

- Owns exactly one Postgres schema (`payments`) and one DB role (`payments_svc`).
