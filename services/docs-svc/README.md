# docs-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** statement and receipt rendering, Excel exports, object storage references.

## Contract every service honours

- Owns exactly one Postgres schema (`docs`) and one DB role (`docs_svc`).
