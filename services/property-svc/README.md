# property-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** properties, units, plots, landlords, management agreements, leases, residents.

## Contract every service honours

- Owns exactly one Postgres schema (`property`) and one DB role (`property_svc`).
