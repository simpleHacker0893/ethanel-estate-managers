# identity-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** organizations, memberships, roles, Clerk user sync, staff and caretaker accounts.

## Contract every service honours

- Owns exactly one Postgres schema (`identity`) and one DB role (`identity_svc`).
