# money-svc — placeholder

Fastify service on `@ethanel/chassis`. Implemented in a later sprint (see `.docs/sprints/`).

**Owns:** the append-only ledger: postings, lease accounts, agency fees per management agreement, landlord statements and remittance schedules.

## Contract every service honours

- Owns exactly one Postgres schema (`money`) and one DB role (`money_svc`).
